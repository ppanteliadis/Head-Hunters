...
<!DOCTYPE html>
<title>Head Hunters - Possible Jobs </title>
<html>
<body>
<!-- FONT AWESOME -->
...
<header class="signup dark-bg" id="signup">
	<div class="color-overlay">
		<div class="swrapper">
		<h1>These are the <span class="colored-text">Available</span> positions</h1>
		<h3> that match your criteria!</h3>
		<?php 
			$sql1 = "SELECT DISTINCT posid FROM open_position, person
					 WHERE person.pid=$s_pid 
					 	AND (person.prof=open_position.field 
			...
			if ($result1 = mysqli_query($dbconn, $sql1)){
				...
				$i = 0;
				while ($i < $size) {
					$sql2 = "SELECT * FROM open_position WHERE posid = '$data[$i]'";
